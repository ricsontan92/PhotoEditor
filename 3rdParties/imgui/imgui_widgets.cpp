    ...
    const int circle_count, const float speed)
{
    ...
    for (int i = 0; i < circle_count; ++i) {
        ...
        const auto growth = std::max(0.0f, sinf(t * speed - i * degree_offset));
        ImVec4 color;
        color.x = main_color.x * growth + backdrop_color.x * (1.0f - growth);
        color.y = main_color.y * growth + backdrop_color.y * (1.0f - growth);
        color.z = main_color.z * growth + backdrop_color.z * (1.0f - growth);
        color.w = 1.0f;
        window->DrawList->AddCircleFilled(ImVec2(pos.x + indicator_radius + (float)x,
            pos.y + indicator_radius - (float)y),
    ...
    const int circle_count, const float speed)
{
    ...
    const float circle_radius = indicator_radius / 10.0f;
    const ImRect bb(position, ImVec2(position.x + indicator_radius * 2.0f,
        position.y + indicator_radius * 2.0f));
    ...
    for (int i = 0; i < circle_count; ++i) {
        ...
        color.z = main_color.z * growth + backdrop_color.z * (1.0f - growth);
        color.w = 1.0f;
        window->DrawList->AddCircleFilled(ImVec2(position.x + indicator_radius + (float)x,
            position.y + indicator_radius - (float)y),
            circle_radius + growth * circle_radius,
            GetColorU32(color));
    }
}
